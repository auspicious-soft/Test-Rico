...
const Page = () => {
  ...
  const { data, isLoading, mutate, error } = useSWR(
    shouldFetchWellness
      ? `/client/${session?.data?.user?.id}/wellness?${query}`
      : null,
    getClientWellness
  );
  ...
  const renderTabContent = (isLoading: boolean) => {
    ...
    if (items.length === 0) {
      return (
        <div className="text-center text-gray-500 py-10">
          {activeTab === "Videos" ? "No Videos Found" : "No Attachments Found"}
        </div>
      );
    }
    switch (activeTab) {
    ...
    <>
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="tab-content">{renderTabContent(isLoading)}</div>
        )}
      </div>
    </>
  );
