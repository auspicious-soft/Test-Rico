...
import { getClientSecretToShowPaymentIntentService, getProfileService } from '@/services/client/client-service';
...
const ViewPlans = (props: any) => {
  let { modalRef,setOpenPlansModal, hideLogout = false, hideSelectYourPlanText = false, onPlansPageWebsite = false } = props;
  const session = useSession();
    const { data: userData, error: userError, isLoading: userLoading } = useSWR( `/client/${session?.data?.user?.id}`, getProfileService, { revalidateOnFocus: false });
  if(userData?.data.data?.planOrSubscriptionId !== null){
    hideLogout= true;
  }
  ...
  return (
    <div className="ma" ref={modalRef}>
      <div className='flex justify-between items-center'>
        ...
        {hideLogout && <a onClick={handleExit} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}> <LogOut className='text-black' /> <span className="text-[#283C63] text-[600] font-bold">Exit</span> </a>}
      </div>
