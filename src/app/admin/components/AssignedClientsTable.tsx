              ...
              assignedData?.map((row: any) => {
                return (
                  <tr key={row?._id}>
                    ...
                    <td className="">{row.assignedDate ? new Date(row.assignedDate).toLocaleDateString() : 'N/A'}</td> 
                    <td className="text-center">{row.planType &&  row.planType !== null ? "True"  : "False"}</td> 
                    {/* Display date */}
                    <td className="">
                      <button
