...
import { format, parse, startOfWeek, getDay, addMinutes, set, isBefore, startOfDay } from "date-fns";
...
const AdminCalendar: React.FC = () => {
  ...
  const onSelectSlot = useCallback((slotInfo: any) => {
    ...
    const slotDate = startOfDay(slotInfo.start); // Start of selected slot date
  ...
  const CustomToolbar: React.FC<ToolbarProps<CalendarEvent, object>> = ({ onNavigate, label }) => {
    const buttonClass = "px-3 py-2 roundup-md transition-colors duration-200 text-sm whitespace-nowrap";
  ...
  return (
    <div className="p-2 md:p-5 min-h-[calc(100vh-200px)]">
      <h2 className="text-xl md:text-2xl font-bold mb-2 md:mb-4">Appointment Calendar</h2>
      {isModalOpen && (
        <EventModal
          event={selectedEvent}
          isOpen={isModalOpen}
