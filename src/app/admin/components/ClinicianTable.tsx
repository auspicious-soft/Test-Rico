              ...
              therapistsDataArray?.map((row: any) => {
                return (
                  <tr key={row?._id}>
                    ...
                    <td>{row?.otherDetailsOfTherapist?.phoneNumber || "-"}</td>
                    <td>
                      {row?.otherDetailsOfTherapist?.addressLine1 ? (
                        <>
                          {row.otherDetailsOfTherapist.addressLine1}
                          {row.otherDetailsOfTherapist.addressLine2 ? `, ${row.otherDetailsOfTherapist.addressLine2}` : ""}
                          {row.otherDetailsOfTherapist.state ? `, ${row.otherDetailsOfTherapist.state}` : ""}
                        </>
                      ) : (
                        "-"
                      )}
                    ...
                    <td>
                      <div className="flex gap-2">
                        <button disabled={row?.otherDetailsOfTherapist?.onboardingPdfKey === undefined} onClick={() => openEditModal(row)} className="">
                          {" "}
                          <EditIcon />
                        </button>
                        <button
                    ...
                    <td>
                      <select
                        name="status"
                        disabled={row?.otherDetailsOfTherapist?.onboardingPdfKey === undefined}
                        value={row?.otherDetailsOfTherapist?.status}
                        onChange={(event) => handleInputChange(event, row?._id)}
                        className="w-[280px] border-none h-auto bg-transparent pl-1"
                      >
                    ...
                    <td className="text-center flex justify-center">
                      {row?.otherDetailsOfTherapist?.onboardingPdfKey !== undefined ?
                        ...
                        :
                        <EyeIcon className="bg-gray-300 text-white rounded-md px-1" />}
                    </td>
                  </tr>
                )
              })
