  ...
  useEffect(() => {
    if (row) {
      setFormData((prevFormData: any) => ({
        ...
        phoneNumber: row?.otherDetailsOfTherapist?.phoneNumber,
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        const valid = validUSPhoneNumber(updatedFormData.phoneNumber)
        if (!valid) {
          toast.warning(validUSPhoneMsg, { duration: 2000 })
          return
        }
      ...
      <form onSubmit={handleFormSubmit}>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="email" disabled name="email" id="" defaultValue={formData.email} onChange={handleInputChange} required />
          </div>
          <div>
