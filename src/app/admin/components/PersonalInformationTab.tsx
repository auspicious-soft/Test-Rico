...
const PersonalInformationTab: React.FC<PersonalInformationTabProps> = ({ row, mutate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const phoneValid = validUSPhoneNumber(formData.phoneNumber);
    if (!phoneValid) {
      ...
      <form onSubmit={handleSubmit}>
        <div className="add-clinician flex flex-wrap gap-[30px]">
          ...
          <div className='selector'>
            ...
            <input disabled type="email" name="email" value={formData.email} onChange={handleChange} placeholder="email@example.com" />
          </div>
          {/* <div className='selector'>
