...
const QuestionComponent: React.FC<QuestionComponentProps> = ({
  ...
  const selectedValues = Array.isArray(formData[name]) ? formData[name] : [];
          ...
          {type === 'radio' && options ? (
            ...
            multiSelect ? (
              ...
              <div className="relative">
                <div
                  className={clsx(
                    "w-full text-[#686C78] px-[18px] h-[45px] text-sm border rounded-[20px] focus:outline-none focus:ring-1 cursor-pointer flex items-center justify-between",
                ...
                {isDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-[#dbe0eb] rounded-[10px] shadow-lg max-h-60 overflow-y-auto">
                    {options.map(option => (
                      <div
                        key={option}
                ...
                {selectedValues.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {selectedValues.map(value => (
                      <span
                        key={value}
                        className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-[#283C63] text-white"
                      >
                        {value}
                        <button
                          type="button"
