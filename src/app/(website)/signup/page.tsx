...
const Page: React.FC = () => {
  ...
  const handleSignup = async () => {
    const isValid = validUSPhoneNumber('+' + String(phoneNumber))
    if (!isValid) {
      toast.warning(validUSPhoneMsg)
      return
    }
    startTransition(async () => {
      try {
        const response = await signUpTherapistService({ firstName, lastName, phoneNumber: `+${phoneNumber}`, email, password })
